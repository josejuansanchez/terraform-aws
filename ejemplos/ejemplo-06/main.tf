...
resource "aws_security_group" "sg_ejemplo_06" {
  name        = var.sg_name
...
resource "aws_security_group_rule" "ingress" {
  security_group_id = aws_security_group.sg_ejemplo_06.id
  type              = "ingress"
...
resource "aws_instance" "instancia_ejemplo_06" {
  ami             = var.ami_id
  instance_type   = var.instance_type
  key_name        = var.key_name
  security_groups = [aws_security_group.sg_ejemplo_06.name]
